...
_c_herdr() {
	if [ "$CONDUCTOR_DRY_RUN" = 1 ]; then
		{
			printf 'DRYRUN: %s' "$HERDR_BIN_PATH"
			printf ' %q' "$@"
			printf '\n'
		} >&2
		return 0
...
_c_version_gate() {
	...
	command -v "$HERDR_BIN_PATH" >/dev/null 2>&1 || {
		_c_die "herdr not found (set HERDR_BIN_PATH)"
		return 1
	}
	local v
	v="$("$HERDR_BIN_PATH" --version 2>/dev/null | grep -Eo '[0-9]+\.[0-9]+\.[0-9]+' | head -1)"
	[ -n "$v" ] || {
		_c_die "cannot read herdr version"
		return 1
	}
	...
	if [ "$a" -lt "$x" ] || { [ "$a" -eq "$x" ] && [ "$b" -lt "$y" ]; } || { [ "$a" -eq "$x" ] && [ "$b" -eq "$y" ] && [ "$c" -lt "$z" ]; }; then
		_c_die "herdr $v < required $CONDUCTOR_MIN_HERDR"
		return 1
	fi
	return 0
}
...
conductor_start_worker() {
	...
	while [ $# -gt 0 ]; do case "$1" in
		...
		--)
			...
			;;
		*)
			_c_die "unknown flag: $1"
			return 1
			;;
		esac done
	_c_slug_ok "$role" || {
		_c_die "role '$role' is not a valid agent name ([a-z][a-z0-9_-]{0,31})"
		return 1
	}
	[ -d "$cwd" ] || {
		_c_die "cwd does not exist: $cwd"
		return 1
	}
...
d=json.load(sys.stdin);p=[x for x in d["result"]["panes"] if x.get("focused")];print(p[0]["pane_id"] if p else "")' 2>/dev/null)"
		[ -n "$anchor" ] || {
			_c_die "no anchor pane (set CONDUCTOR_ANCHOR_PANE)"
			return 1
		}
	fi
	: "${anchor:=DRYPANE}"
...
try: print(json.load(sys.stdin)["result"]["pane"]["pane_id"])
		...
		[ -n "$pane" ] || {
			_c_die "pane split failed: $split"
			return 1
		}
		...
		for t in $(seq 1 25); do
			...
			printf '%s' "$out" | grep -q 'agent_pane_busy' && {
				sleep 1
				continue
			}
			break
		...
		if printf '%s' "$out" | grep -q '"error"'; then
			...
			_c_die "agent start failed for '$role' after $t tries: $out"
			return 1
		fi
	fi
...
conductor_dispatch() {
	...
	local sf
	sf="$(_c_role_state "$role")"
	[ -f "$sf" ] || {
		_c_die "no such worker: $role"
		return 1
	}
	[ -f "$taskfile" ] || {
		_c_die "task file not found: $taskfile"
		return 1
	}
...
conductor_await() {
	...
	_c_die "worker '$role' settled without a complete report after re-prompt — escalate to human"
	return 2
}
...
conductor_collect() {
	...
	. "$sf"
	[ "$CONDUCTOR_DRY_RUN" = 1 ] && {
		echo "DRYRUN: collect $role"
		return 0
	}
	local report="$CWD/.conductor/report.md"
	[ -f "$report" ] || {
		_c_die "collect('$role'): report.md absent"
		return 1
	}
	[ -s "$report" ] || {
		_c_die "collect('$role'): report.md empty"
		return 1
	}
	grep -q "$CONDUCTOR_SENTINEL" "$report" || {
		_c_die "collect('$role'): report.md missing sentinel"
		return 1
	}
	local m
	m="$(_c_mtime "$report")"
	[ "$m" -ge "$DISPATCH_TS" ] || {
		_c_die "collect('$role'): report.md is stale (mtime<dispatch) — refusing stale result"
		return 1
	}
	cat "$report"
}
...
print(v if isinstance(v, str) else "")' 2>/dev/null)"
	...
	top="$(git -C "$dir" rev-parse --show-toplevel 2>/dev/null)" || {
		_c_die "$dir is not a git repository — run the conductor from a repo workspace"
		return 1
	}
	printf '%s\n' "$top"
}
...
_c_repo_or_pwd() {
	local r
	if r="$(conductor_repo_root 2>/dev/null)"; then
		printf '%s\n' "$r"
		return 0
	fi
	if [ "$CONDUCTOR_DRY_RUN" = 1 ]; then
...
conductor_config_load() {
	...
	if [ -z "$path" ]; then
		local repo
		repo="$(conductor_repo_root)" || return 1
		path="$repo/$CONDUCTOR_CONFIG_NAME"
	fi
	[ -f "$path" ] || {
		_c_die "team config not found: $path"
		return 1
	}
...
conductor_worktree() {
	...
	mkdir -p "$wtroot" || {
		_c_die "cannot create the worktree root: $wtroot"
		return 1
	}
	local out
	...
	[ -d "$wt" ] || {
		_c_die "worktree add failed for '$role': $out"
		return 1
	}
	printf '%s\n' "$wt"
}
...
conductor_guard_drop() {
	local cwd="${1:-}"
	[ -d "$cwd" ] || {
		_c_die "guard drop: cwd does not exist: $cwd"
		return 1
	}
	local f="$cwd/.herdr-guard.json"
	if [ "$CONDUCTOR_DRY_RUN" = 1 ]; then
		printf 'DRYRUN: write %q\n' "$f" >&2
		return 0
	fi
...
conductor_assemble() {
	_c_version_gate || return 1
	local norm
	norm="$(conductor_config_load "${1:-}")" || return 1
	mkdir -p "$(_c_run_dir)"
	printf '%s\n' "$norm" >"$(_c_team_json)"
...
for r in json.load(sys.stdin)["roles"]:
    print("\t".join([r["name"], r["kind"], r["mode"], r["branch"],
		...
		_c_die "cannot expand the team roles"
		return 1
	}
	...
	while IFS="$(_c_tab)" read -r name kind mode branch wants_wt wants_guard args; do
		[ -n "$name" ] || continue
		if [ "$wants_wt" = 1 ]; then
			cwd="$(conductor_worktree "$name" "$base" "$wtroot" "$branch")" || {
				rc=1
				continue
			}
		else
...
conductor_render_role() {
	...
	tj="$(_c_team_json)"
	[ -f "$tj" ] || {
		_c_die "no assembled team — run conductor_assemble first"
		return 1
	}
	local out
	out="$(_c_run_dir)/$role.task.md"
	CONDUCTOR_ROLE="$role" \
...
conductor_reconcile() {
	...
	while [ $# -gt 0 ]; do case "$1" in
		...
		--branch)
			...
			;;
		*)
			_c_die "conductor_reconcile: unknown flag: $1"
			return 1
			;;
		esac done
	local tj
	...
	local branches
	branches="$(printf '%s' "$norm" | python3 -c 'import json,sys
for r in json.load(sys.stdin)["roles"]:
    if r["mode"] == "write": print("%s\t%s" % (r["name"], r["branch"]))')"
	[ -n "$branches" ] || {
		echo "reconcile: no writer roles to merge"
		return 0
	}
	...
	if [ ! -d "$into" ]; then
		...
		[ -d "$into" ] || {
			_c_die "cannot create the integration worktree $into: $wout"
			return 1
		}
	fi
