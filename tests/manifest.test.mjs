...
function fixture() {
  const base = fs.mkdtempSync(path.join(os.tmpdir(), "herdr-conductor-manifest-"));
  const repository = path.join(base, "repository");
  fixtures.push(base);
  fs.mkdirSync(path.join(repository, "scripts"), { recursive: true });
  fs.writeFileSync(path.join(repository, "package.json"), JSON.stringify({ version: "1.2.3" }));
  fs.writeFileSync(
...
test("manifest validation rejects malformed TOML", () => {
  ...
  fs.writeFileSync(path.join(repository, "herdr-plugin.toml"), 'version = "1.2.3"\n[[actions]\n');

  const { errors } = validateRepository(repository);
  assert.ok(errors.some((error) => error.startsWith("manifest is not valid TOML:")), errors.join("\n"));
});

test("manifest validation enforces package version parity", () => {
  const { repository } = fixture();
  fs.writeFileSync(path.join(repository, "package.json"), JSON.stringify({ version: "9.9.9" }));

  const { errors } = validateRepository(repository);
  assert.ok(errors.some((error) => error.startsWith("version mismatch:")), errors.join("\n"));
});
...
test("manifest validation requires action entrypoints to exist", () => {
  ...
  const { errors } = validateRepository(repository);
  assert.ok(errors.some((error) => error.includes("entrypoint does not exist")), errors.join("\n"));
});
...
test("manifest validation keeps action entrypoints inside the repository", () => {
  ...
  const { errors } = validateRepository(repository);
  assert.ok(errors.some((error) => error.includes("entrypoint escapes the repository")), errors.join("\n"));
});
...
test("manifest validation requires executable scripts", () => {
  ...
  const { errors } = validateRepository(repository);
  assert.ok(errors.some((error) => error.includes("script is not executable")), errors.join("\n"));
});
