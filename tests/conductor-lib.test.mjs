...
test("start_worker emits pane split then agent start", () => {
	...
	assert.ok(
		r.stderr.indexOf("pane split") < r.stderr.indexOf("agent start"),
		"split must precede start",
	);
});
...
test("start_worker passes agent argv through", () => {
	const cwd = tmp();
	const r = runLib(
		`conductor_start_worker reviewer codex "${cwd}" -- --sandbox read-only`,
	);
	assert.equal(r.status, 0, r.stderr);
...
test("collect fails when the report is absent", () => {
	...
	const r = runLib("conductor_collect builder", {
...
test("config rejects an unknown mode", () => {
	const r = loadConfig({
		version: 1,
		roles: [{ name: "x", kind: "claude", mode: "sudo" }],
	});
	assert.notEqual(r.status, 0);
...
test("config rejects a role name herdr could not use as an agent name", () => {
	const r = loadConfig({
		version: 1,
		roles: [{ name: "Builder Engine", kind: "claude" }],
	});
	assert.notEqual(r.status, 0);
...
test("config rejects launch_args that are not strings", () => {
	const r = loadConfig({
		version: 1,
		roles: [{ name: "b", kind: "claude", launch_args: [1, 2] }],
	});
	assert.notEqual(r.status, 0);
...
test("assemble emits the full herdr sequence for every role", () => {
	...
	const state = tmp("cond-state-");
	const r = runLib(`conductor_assemble "${cfg}"`, {
		CONDUCTOR_REPO: repo,
		CONDUCTOR_STATE_DIR: state,
	});
	assert.equal(r.status, 0, r.stderr);
	...
	assert.match(
		r.stderr,
		/agent start validator --kind codex .*-- --sandbox workspace-write/,
	);
	assert.match(
		r.stderr,
		/agent start reviewer --kind codex .*-- --sandbox read-only/,
	);
	...
	for (const role of ["builder-engine", "validator", "reviewer"])
		assert.match(r.stdout, new RegExp(role));

	// the normalized team is recorded for render_role / reconcile to read back
	const team = JSON.parse(
		readFileSync(join(state, "run-t", "team.json"), "utf8"),
	);
...
test("a dry-run assemble avoids Herdr and Git mutations but creates transport files", () => {
	...
	assert.equal(
		...
		1,
		"dry run created a worktree",
	);
	assert.ok(
		!existsSync(join(repo, ".herdr-guard.json")),
		"dry run wrote a guard policy",
	);
	assert.equal(
		git(repo, "branch", "--list", "conductor/*"),
		"",
		"dry run created a branch",
	);
	assert.ok(
...
function recordedTeam(cfg = THREE_ROLES) {
	...
	const r = runLib(
		`conductor_config_load "${p}" > "${join(state, "run-t", "team.json")}"`,
		{
			CONDUCTOR_REPO: repo,
			CONDUCTOR_STATE_DIR: state,
		},
...
test("render_role fills every slot and appends the report contract", () => {
	...
	assert.ok(
		task.trimEnd().endsWith(SENTINEL),
		"the report sentinel contract was not appended",
	);
});
...
test("render_role errors on an unknown role and a missing contract", () => {
	...
	const missing = runLib(
		"conductor_render_role builder-engine /no/such/contract.md",
		env,
	);
...
test("worktree creates a role branch, and reuses it on a second call", () => {
	...
	assert.equal(
		git(repo, "branch", "--list", "conductor/builder-engine")
			.replace(/^[+*]\s*/, "")
			.trim(),
	...
	assert.ok(
		...
		"reuse clobbered in-progress work",
	);
});
...
test("conductor artifacts never show up in the user's git status", () => {
	...
	const env = { CONDUCTOR_REPO: repo, CONDUCTOR_DRY_RUN: "0" };
	assert.equal(
		runLib("conductor_worktree builder-engine main .conductor-worktrees", env)
			.status,
		0,
	);
	...
	assert.equal(
		...
		"",
		"conductor artifacts leaked into git status",
	);
	assert.ok(
		!existsSync(join(repo, ".gitignore")),
		"the repo's own .gitignore was edited",
	);
});
...
test("guard drop writes an alert-only substring policy and never clobbers", () => {
	...
	for (const rule of policy.rules) {
		assert.equal(
			rule.match,
			"substring",
			`rule ${rule.id} uses a non-substring match`,
		);
		assert.equal(
			rule.severity,
			"alert",
			`rule ${rule.id} exceeds the alert cap`,
		);
	}
	...
	assert.equal(
		again.stdout.trim(),
		"",
		"guard drop reported writing over an existing policy",
	);
	assert.deepEqual(
		JSON.parse(readFileSync(join(cwd, ".herdr-guard.json"), "utf8")).rules,
		[],
	);
});
...
function reconcileFixture({ collide }) {
	...
	for (const [role, file, body] of [
		...
		const wt = runLib(
			`conductor_worktree ${role} main .conductor-worktrees`,
			env,
		).stdout.trim();
...
test("reconcile reports a conflict, returns non-zero, and leaves no half-merge", () => {
	...
	assert.equal(
		git(into, "status", "--porcelain=v1"),
		"",
		"merge --abort left the tree dirty",
	);
});
...
test("reconcile flags a writer branch that was never created", () => {
	...
	git(
		...
		"remove",
		"--force",
		join(repo, ".conductor-worktrees", "builder-ui"),
	);
...
test("status reads live state from `agent list`, not from pane text", () => {
	...
	const bin = fakeHerdr(
		JSON.stringify({
			result: {
				agents: [
					{ name: "builder", agent_status: "working" },
					{ name: "reviewer", agent_status: "idle" },
				],
			},
		}),
	);
	const r = runLib("conductor_status", {
		CONDUCTOR_STATE_DIR: state,
		CONDUCTOR_DRY_RUN: "0",
		HERDR_BIN_PATH: bin,
	});
...
test("repo_root fails loudly outside a repo instead of picking a nearby one", () => {
	const notARepo = tmp();
	const r = runLib("conductor_repo_root", {
		CONDUCTOR_REPO: notARepo,
		CONDUCTOR_DRY_RUN: "0",
	});
	assert.notEqual(r.status, 0);
